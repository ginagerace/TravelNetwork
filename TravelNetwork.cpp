...
class TravelNetwork {
 private:
   ...
   //recursive function called by depthFirthSearchHelper()
   bool dfs(int a, int b) {
     for(int i=1; i<=sizeArr; i++) {
        if(airArr[a].contains(i) == true) {         //check if reachable from airport a
            if(i == b)
                return true;
            if(airArr[i].getVisited() == false){    //check if unvisited
                airArr[i].setVisited(true);         //mark as visited
                if(dfs(i, b) == true)               //recursive call
                    return true;
            }
        }
     }
